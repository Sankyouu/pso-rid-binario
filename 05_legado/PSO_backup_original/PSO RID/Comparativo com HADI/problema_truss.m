...
% --- TENSÃO ADMISSÍVEL ---
sigma_max = 172.25; % [MPa] — tração e compressão (artigo, Seção 6.1)
...
% --- MULTI-START ---
n_runs             = 10;   % Mais runs dado o espaço discreto com múltiplos mínimos
melhor_peso_global = inf;
melhor_sol_global  = [];
todos_historicos   = {};

funcao_para_solver = @(x) custo_truss(x, sigma_max);
...
J = weight; % Peso em kg — o Método de Deb no solver trata a restrição via violação
end